    ...
    <div className="fixed inset-0 z-50 overflow-hidden flex justify-end">
      ...
      <div className="relative w-full max-w-2xl bg-[#fcfcfb] h-full shadow-2xl z-10 flex flex-col overflow-hidden animate-slideLeft border-l border-slate-200/90">
        ...
        <div className="px-6 py-5 bg-white border-b border-slate-200/80 flex items-start justify-between gap-4 shrink-0">
          <div>
            {/* Prominent Booking Reference & Status */}
            <div className="flex flex-wrap items-center gap-2 mb-2">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-brand-sky-50 border border-brand-sky-200/90 shadow-2xs">
                <span className="text-[10px] font-bold uppercase tracking-wider text-brand-sky-700">
                  Booking Reference:
                </span>
                <span className="font-mono text-xs font-extrabold text-brand-navy-950 tracking-wider">
                  {enquiry.id}
                </span>
                <button
                  type="button"
                  onClick={() => handleCopy(enquiry.id, 'refId')}
                  className="p-1 -mr-1 hover:bg-brand-sky-100 rounded text-brand-sky-700 transition-colors flex items-center gap-1"
                ...
                >
                  {copiedField === 'refId' ? (
                    ...
                    <Copy className="w-3.5 h-3.5 text-brand-sky-600" />
                  )}
                </button>
              </div>
