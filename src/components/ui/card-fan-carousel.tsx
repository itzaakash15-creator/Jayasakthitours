      ...
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem]">
          {cards.map((card, index) => {
            const image = (
              <div className="relative w-full h-full overflow-hidden rounded-[1.25rem] sm:rounded-[1.5rem] lg:rounded-[1.75rem]">
                <img
                  src={card.imgUrl}
                  loading="lazy"
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-brand-navy-950/80 via-transparent to-transparent z-20 pointer-events-none" />
                {card.alt && (
                  <div className="absolute bottom-3 left-3 right-3 z-30 pointer-events-none">
                    <p className="text-white text-xs sm:text-sm font-bold drop-shadow-md line-clamp-1">
                      {card.alt}
                    </p>
                  </div>
                )}
      ...
      {needsPagination && (
        <div className="flex items-center justify-center gap-4 mt-6 md:mt-8 z-30">
          ...
          <div className="flex items-center gap-2">
            {cards.map((_, i) => (
              <span key={i} className={`w-2 h-2 rounded-full transition-all duration-300 ${i === centerIndex ? "bg-brand-sky-600 scale-[1.4]" : "bg-black/20"}`} />
            ))}
          </div>
